...
export default {
  ...
  methods: {
    ...
    // 选择项发生变化
    parentCateChange() {
      // 如果selectCate数组中的length>0说明选中的父级分类
   ...
   this.selectCate = []
    ...
    // 监听修改分类对话框关闭清空表单数据
    editCateDialogClose(){
        this.$refs.editCateFormRulesRef.resetFields()
    },
    // 修改分类的弹窗
    ...
    // 修改分类
    editCate(){
      this.$refs.editCateFormRulesRef.validate(async valid=>{
        if(!valid)return 
